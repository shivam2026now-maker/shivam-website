...
export class RuntimeCommand extends Command {
    ...
    renderWarnings(warnings) {
        if (!warnings.length)
            return;
        this.log('');
        this.log(styleText(['bold', 'yellow'], 'Notice'));
        this.log('');
...
export class ResolvedCommand extends RuntimeCommand {
    ...
    stackId;
    /**
     * locally-resolved Stack ID (flags > env > module > config)
     * populated regardless of `needs` and is never required
     */
    resolvedStackId;
    /**
     * locally-resolved scope (flags > env > module > config)
     * populated regardless of `needs` and is never required
     */
    resolvedScopeType;
    resolvedScopeId;
    /**
     * the located `.sanity/blueprint.config.json` (if any), populated regardless of
     * `needs`. Lets commands read/update local config without requiring a manifest.
     */
    blueprintConfig;
    auth;
    deployedStack;
    /** Runs after flag parsing and before declared command context is resolved. */
    async preResolve() { }
    async init() {
        ...
        // Expose the resolved Stack ID and scope regardless of `needs` (optional, for listings/highlighting)
        this.resolvedStackId = resolved.stackId;
        this.resolvedScopeType = resolved.scopeType;
        this.resolvedScopeId = resolved.scopeId;
        ...
        if (needsScope) {
            if (!resolved.scopeType || !resolved.scopeId) {
                // In CI environments, since .sanity/blueprint.config.json is not guaranteed to exist, we can provide a more specific hint
                const errorMessage = config.isCi
                    ? 'set SANITY_PROJECT_ID or SANITY_ORGANIZATION_ID.'
                    : 'provide --project-id or --organization-id, or run `blueprints init` to create a Blueprint config (.sanity/blueprint.config.json).';
                this.error(`Missing scope: ${errorMessage}`, {
                    suggestions: [
                        `Run \`${this.config.bin} blueprints doctor\` to check your configuration.`,
                    ],
        ...
        if (needsStack) {
            ...
            if (!stackId) {
                const errorMessage = config.isCi
                    ? "couldn't determine which Stack to target. Set SANITY_BLUEPRINT_STACK_ID."
                    : `provide --stack, or set a Stack in your Blueprint config (\`${this.config.bin} blueprints config --edit\`).`;
                this.error(`Missing Stack: ${errorMessage}`, {
                    suggestions: [
                        `Run \`${this.config.bin} blueprints doctor\` to check your configuration.`,
                    ],
                    ref: CI_SETUP_DOCS_URL,
