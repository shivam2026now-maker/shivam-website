import { validateCorsOrigin, validateDataset, validateDocumentWebhook, validateDurableFunction, validateProject, validateRobot, validateRole, } from '@sanity/blueprints';
import { SANITY_FUNCTION_PREFIX, SANITY_FUNCTION_PUBSUB, SANITY_STUDIO } from '../constants.js';
export function isLocalFunctionResource(r) {
    return r.type.startsWith(SANITY_FUNCTION_PREFIX);
}
export function hasEventSource(r) {
    return r.type !== SANITY_FUNCTION_PUBSUB;
}
...
export function isStudioResource(r) {
    return r.type === SANITY_STUDIO;
}
export function isDurableFunctionResource(r) {
    return validateDurableFunction(r).length === 0;
}
