...
export class Undeploy extends SanityCommand {
    ...
    async run() {
        ...
        if (!dataset) {
            this.error('Dataset is required. Pass it with `--dataset <name>` or configure it in your project.', {
        ...
        if (!force) {
            ...
            const confirmMessage = tag === 'default' ? `Delete the GraphQL API for dataset "${dataset}" in project ${projectId}?` : `Delete the GraphQL API for dataset "${dataset}" in project ${projectId} with tag "${tag}"?`;
