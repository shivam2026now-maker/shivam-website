...
export class MediaCreateAspectCommand extends SanityCommand {
    ...
    async run() {
        ...
        const title = this.flags.title ?? (this.isUnattended() ? this.error('Aspect title is required. Pass it with `--title <title>`.', {
            exit: exitCodes.USAGE_ERROR
        }) : await input({
            message: 'Title'
        }));
        const defaultName = createPublishedId(camelCase(title));
        const name = this.flags.name ?? (this.isUnattended() ? defaultName : await input({
            default: defaultName,
            message: 'Name'
        }));
        try {
