import { hasEventSource, isLocalFunctionResource, isScheduleEvent, } from '../../utils/types.js';
...
export function arrayifyCors(resource) {
    const details = [formatLabeledValue('origin', resource.origin)];
    if (resource.allowCredentials !== undefined) {
        details.push(formatLabeledValue('allowCredentials', resource.allowCredentials));
    }
    return details;
}
export function arrayifyDataset(resource) {
    const details = [formatLabeledValue('title', resource.datasetName)];
    if (resource.aclMode)
        details.push(formatLabeledValue('visibility', resource.aclMode));
    if (resource.project)
        details.push(formatLabeledValue('project', niceId(resource.project)));
    return details;
}
function arrayifyEvent(event) {
    ...
    if (isScheduleEvent(event)) {
        if ('expression' in event) {
            ...
            details.push(formatLabeledValue('expression', `${event.minute} ${event.hour} ${event.dayOfMonth} ${event.month} ${event.dayOfWeek}`));
        }
    }
    else {
...
export function arrayifyStudio(resource) {
    const details = [formatLabeledValue('appHost', resource.slug)];
    if (resource.project)
        details.push(formatLabeledValue('project', niceId(resource.project)));
    return details;
}
export function arrayifyWebhook(resource) {
    ...
    if (resource.displayName)
        details.push(formatLabeledValue('displayName', resource.displayName));
    if (resource.description)
        details.push(formatLabeledValue('description', resource.description));
