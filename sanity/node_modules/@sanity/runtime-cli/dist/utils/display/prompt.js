...
/** Thrown when the user picks "Cancel" in a prompt. Cores translate it to a clean result. */
...
// --- Standalone prompts ---
/**
 * Ask where a new Blueprint should go.
 * Returns the chosen directory, or null to use the current one.
 * @throws CancelledError when the user cancels.
 */
export async function promptForInitLocation(currentDir) {
    const HERE = 'here';
    const ELSEWHERE = 'elsewhere';
    const CANCEL = 'cancel';
    const choice = await select({
        message: 'Where should the Blueprint go?',
        choices: [
            { name: 'Here', value: HERE, description: currentDir },
            { name: 'In a new folder', value: ELSEWHERE, description: 'Name a folder to create' },
            { name: 'Cancel', value: CANCEL, description: 'Stop here and change nothing' },
        ],
        default: HERE,
    });
    if (choice === CANCEL)
    ...
    const folder = await input({
        message: 'Folder name:',
        validate: (val) => val.trim().length > 0 || 'A folder name is required',
    });
    return folder.trim();
}
        ...
        choices: [
            {
                name: 'TypeScript',
                value: 'ts',
                description: 'Editor completion and type checks. Adds @sanity/blueprints.',
            },
            {
                name: 'JavaScript',
                value: 'js',
                description: 'The same define* helpers, without type checks. Adds @sanity/blueprints.',
            },
            {
                name: 'JSON',
                value: 'json',
                description: 'No dependency and no install step. Write each resource by hand.',
            },
        ],
        default: 'ts',
    });
 ...
 */
export async function promptForScope({ allowBack = false, hasProjects = true, message = 'How would you like to scope your Blueprint?', } = {}) {
    const choices = [
        {
            name: 'Organization',
            value: 'organization',
...
/** Offers a recovery path after the selected scope cannot be read. */
export async function promptForScopeRecovery() {
    return await select({
        message: 'What would you like to do?',
        choices: [
            {
                name: 'Choose another organization or project',
                value: 'retry',
            },
 ...
 */
export async function promptForStack({ scopeType, scopeId, token, logger, allowBack = false, suggestedName, }) {
    if (!scopeId) {
    ...
    if (stacks.length > 0) {
        ...
        const proceed = await select({
            message: 'This scope has no Stacks yet:',
            choices: withBackChoice([
                {
                    name: 'Create the first Stack',
                    value: NEW_STACK_ID,
                    description: 'A Blueprint needs a Stack before it can deploy',
                },
                { name: 'Cancel', value: CANCEL, description: 'Stop here and change nothing' },
            ], allowBack),
        });
    ...
    if (pickedStackId === NEW_STACK_ID) {
        // only suggest a free name; the API rejects a duplicate within a scope
        const isTaken = stacks.some((s) => s.name === suggestedName);
        logger(styleText('dim', 'The name is a label. Sanity assigns the Stack ID.'));
        const stackName = await input({
            message: 'Name your new Stack:',
            default: isTaken ? undefined : suggestedName,
 ...
 */
export async function runScopeAndStackWizard({ token, knownOrganizationId, knownProjectId, logger, suggestedStackName, }) {
    const steps = [
        {
            key: 'organization',
            prompt: async () => {
        ...
        },
        {
            ...
            prompt: async (results) => {
                ...
                return await promptForStack({
                    ...
                    logger,
                    allowBack: true,
                    suggestedName: suggestedStackName,
                });
            },
        },
    ];
