# @sanity/groq-condition-describe

## 0.4.1

### Patch Changes

- 210ca63: The API documentation shipped in every package's `.d.ts` files is rewritten to be substantially terser: multi-paragraph doc comments are cut to the facts a consumer needs, and internal narration no longer appears in editor hover text. Runtime behavior, exports, and type signatures are unchanged across the whole package group.

  **No upgrade action required.**

  **Docs impact: None** — the published documentation site is authored separately; only in-editor `.d.ts` hover text changes.

## 0.4.0

### Minor Changes

- 0bc4328: Render diagram tooltips in the Studio's own idiom, through one seam instead of six hand-assembled bodies:
  - Pin the chrome once (padding 2, radius 3, shadow 2, 280px sentence cap / 400px for dev mode's label-value grids). Bodies no longer carry their own outer padding, which had stacked a `padding={3}` box inside the tooltip's own padding.
  - A title with nothing under it renders in regular weight — it is the whole message, not a heading. Semibold is reserved for a title that actually heads content, one per tooltip: gate headings now separate from their checklist rows by full-strength versus muted text rather than a second bold.
  - Checklists, the grouped-edge roster, the group vertex's stage list and an activity's causal waits render as real `<ul>`/`<li>` rows, so wrapping and nesting come from the browser. Previously these were flattened into padded strings with `white-space: pre-wrap`, which left a wrapped row's continuation aligned under its own marker. Each row leads with its glyph in a fixed-width column and hangs its wrapped lines back to the text: one column for a bullet and a verdict alike, where a `list-style-type` marker box is sized by its own content and put the two on different left edges within one tooltip. The glyph is inline content rather than a marker, so it is selected and copied with the row it belongs to.
  - Space the body on one three-step rhythm, named by the role a gap plays: independent blocks (the activity roster, each gate) separate furthest, a lead-in sits closer to the list it introduces, and rows within a list sit closest. A stage tooltip nests several of these, and at one flat step the whole explanation read as a single mass.
  - `MetaTable` renders rows only; the title and description come from the shared body, so dev-mode and end-user tooltips head identically.

  `OUTCOME_MARKS` and `NEUTRAL_MARK` are now exported (from `@sanity/groq-condition-describe`, re-exported by `@sanity/workflow-engine`), so a structured checklist renderer and `checklistLines` draw their verdict glyphs from one vocabulary.

## 0.3.0

### Minor Changes

- f9389e5: New `atomReadsDataset(atom)` — the per-atom refinement of `ConditionAnalysis.readsDataset`, so clause-level reasoning can tell which leg of a boolean tree depends on dataset state (`*[...]` or a deref) rather than scope vars alone.

### Patch Changes

- df4bd80: Docs: an unbound start-context binding or dangling `$fields` condition read evaluates to GROQ null per read — whether the expression then fails closed or passes vacuously depends on where the null lands, so blanket "fails closed" / "silently never passes" claims are corrected to the per-shape truth across the start-context TSDoc, the deploy-rejection messages (now "silently misevaluates"), and the reference doc. The condition-evaluation module header in `groq-condition-describe` states the same per-read truth ("'unevaluable' only where the null lands as the final result"). A test now pins the genuinely-null-result unevaluable case (an ordered comparison against a missing attribute — total `==` equality can only produce a definite false). No evaluation behavior change.

## 0.2.0

### Minor Changes

- 7510fd5: Three correctness fixes in the frontier and counterfactual machinery. `frontierSummary` now renders the unmet frontier as a clause TREE with the right connective at every level — `($a && $b) || $c` all-false reads "Needs one of: ($a must be true and $b must be true) | $c" instead of flat-joining every atom under the root's connective (which claimed $a alone would unblock it, contradicting the pivotal flags shipped beside it). The pruned tree is exposed as a new `ConditionInsight.frontier` field, and `blockedBy` derives from it unchanged. A root double negation (`!!$x`, the common truthiness idiom) no longer reports a blocked condition as satisfied with an empty `blockedBy`: GROQ negation is strict, so the root atom now takes the engine verdict instead of re-coercing. Finally, `whatIfCondition` counterfactuals assign negative array indices the way GROQ reads them (from the end) instead of silently setting a string property; an index that resolves before the array's start throws instead of no-oping.

## 0.1.0

### Minor Changes

- bae7eb8: `checklistLines` takes an optional `{marks}` flag: `marks: false` renders neutral bullets instead of ✓/✗/? outcomes — the no-instance (static) view, where GROQ's total equality yields real-looking `false` against an empty scope and a ✗ would read as a live judgment.
- 7217e14: New package: describe GROQ boolean expressions without an LLM. `analyzeCondition` (AST reads + and/or clause tree in negation normal form), `explainCondition` (three-valued per-atom verdicts against a dataset + params, unmet frontier, pivotality, solvable requirements), `whatIfCondition`/`withAssignment` (counterfactual re-evaluation, fabrication-clamped), and `describeCondition`/`checklistLines` (deterministic phrasing as `InsightPhrase {id, params, text}` for i18n, with `DescribeContext` seams — `renderRead`, `renderRequirement`, `atomPatterns`, `nowParam` — for host vocabularies). Extracted from `@sanity/workflow-engine`'s insight machinery; works on any GROQ filter.
