...
import { CancelledError, promptForBlueprintType, promptForInitLocation, promptForOrganization, promptForProjectInOrg, promptForStack, runScopeAndStackWizard, } from '../../utils/display/prompt.js';
import { isInteractive } from '../../utils/interactive.js';
import { installCommand, runCommand } from '../../utils/package-manager.js';
...
export async function blueprintInitCore(options) {
    ...
    const explicitDir = argDir || flagDir;
    const isHere = !explicitDir || explicitDir === '.' || explicitDir === './';
    let blueprintDir = isHere ? '.' : explicitDir;
    let userProvidedDirName = isHere ? undefined : explicitDir;
    ...
    try {
        ...
        if (existingBlueprintFile) {
            const result = await handleExistingBlueprint({
            ...
            // the config core owns the wizard, but the user ran `init`
            if (!result.success && result.cancelled) {
                return { success: false, error: 'Initialization cancelled.', cancelled: true };
            }
            return result;
        }
        ...
        if (!explicitDir && isInteractive) {
            const chosenDir = await promptForInitLocation(cwd());
            if (chosenDir) {
                blueprintDir = chosenDir;
                userProvidedDirName = chosenDir;
                // findBlueprintFile walks up, so only refuse on a manifest in the folder itself
                const existing = findBlueprintFile(chosenDir);
                if (existing && dirname(resolve(existing.blueprintFilePath)) === resolve(chosenDir)) {
                    return {
                        success: false,
                        error: `A Blueprint already exists in "${chosenDir}".`,
                        suggestions: [
                            `Run \`${runCommand(bin)} blueprints init ${chosenDir}\` to configure it for deployment.`,
                        ],
                    };
                }
            }
...
async function handleExampleInitialization(options) {
    ...
    log(`\n  Run "${styleText(['bold', 'magenta'], `cd ${exampleDir} && ${installCommand(exampleDir)}`)}" and check out the README`);
    if (instructions) {
        log('');
        log(instructions);
    }
...
export async function resolveScopeAndStack(params) {
    ...
    const suggestedName = params.blueprintDir ? basename(resolve(params.blueprintDir)) : undefined;
    ...
    const wizard = await runScopeAndStackWizard({
        token,
        logger: log,
        suggestedStackName: suggestedName,
    });
...
export async function createBlueprintFiles(params) {
    ...
    const runLine = (command, purpose) => `  Run "${styleText(['bold', 'magenta'], command)}" ${purpose}`;
    // project setup, only when there is something to do
    const setupParts = [];
    if (userProvidedDirName)
        setupParts.push(`cd ${userProvidedDirName}`);
    if (blueprintExtension !== 'json')
        setupParts.push(installCommand(blueprintDir));
    log('');
    if (setupParts.length > 0)
        log(runLine(setupParts.join(' && '), 'first'));
    // the manifest starts empty, so name the command that fills it
    log(runLine(`${runCommand(bin)} functions add`, 'to add a Function'));
    log(runLine(`${runCommand(bin)} blueprints --help`, 'to see every command'));
    return { success: true };
}
