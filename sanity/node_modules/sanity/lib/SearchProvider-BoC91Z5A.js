import { Ar as EMPTY_ARRAY, Cn as useDocumentPresence, En as DocumentVersionsStatus, Ln as useSearchMaxFieldDepth, Nn as defineSearchMachine, Pn as PreviewCard, Rn as createSearch, Tn as DocumentVersionsStatusIndicator, Wn as supportsTouch, _n as getPreviewStateObservable, wn as DocumentPreviewPresence } from "./PerspectiveProvider-CVaQBlks.js";
import { Kt as useSchema, a as useGrantsStore, l as useProjectStore, r as useDocumentPreviewStore, s as useKeyValueStore, tn as isRecord, u as useRenderingContextStore } from "./datastores-APxbZtSi.js";
...
import { $ as useDocumentVersions, a as useValuePreview, h as SanityDefaultPreview, t as useActiveReleases } from "./useActiveReleases-Dn5i8ywb.js";
...
import { c as truncateString } from "./ReleaseAvatar-DE05dW_r.js";
import { r as Popover } from "./MenuGroup-BUur6KUW.js";
...
function useRenderingContext() {
	let renderingContextStore = useRenderingContextStore();
...
function useSearch(t0) {
	...
	let searchState = useSelector(actorRef, t10, isEqual), t11;
...
function _temp2$2(t0) {
	...
	return isEqual(context.query, event.query);
}
function _temp$6() {
...
const rotate = keyframes`from{transform:rotate(0deg);}to{transform:rotate(360deg);}`, AnimatedSpinnerIcon = styled(SpinnerIcon).withConfig({
	...
	componentId: "sc-9krdmn-0"
})`animation:${rotate} 500ms linear infinite;`, FilterDiv = styled.div.withConfig({
	displayName: "FilterDiv",
	componentId: "sc-9krdmn-1"
	...
	return $[11] !== fieldDefinitions || $[12] !== filterDefinitions || $[13] !== operatorDefinitions || $[14] !== schema || $[15] !== setStoredSearch || $[16] !== storedSearch ? (t5 = {
		addSearch: (searchTerm, searchFilters) => {
			let validStoredFilters = (searchFilters || []).map(_temp$3).filter((filter_0) => validateFilter({
				...
				version: 3,
				recentSearches: [newSearchItem, ...storedSearch.recentSearches.filter((r) => !isEqual(omit(r, "created"), omit(newSearchItem, "created")))].slice(0, 5)
			};
...
function CustomMenuItem(t0) {
	...
	let handleClick = t2, t3;
	$[3] !== currentOrdering || $[4] !== ordering ? (t3 = isEqual(currentOrdering, ordering), $[3] = currentOrdering, $[4] = ordering, $[5] = t3) : t3 = $[5];
...
function SortMenu() {
	...
	if ($[10] !== menuOrderings || $[11] !== ordering || $[12] !== t) {
		...
		bb0: {
			let currentMenuItem = menuOrderings.find((item) => isEqual(ordering, item) && !isSearchDivider(item));
			if (!currentMenuItem) {
				t13 = null;
				break bb0;
			}
...
function debugWithName(name) {
	...
	return createDebug && createDebug.enabled(namespace) ? createDebug(namespace) : createDebug(rootName);
}
...
function SearchProvider(t0) {
	...
	$[52] !== completeFilters.length || $[53] !== cursor || $[54] !== documentTypes || $[55] !== handleSearch || $[56] !== ordering || $[57] !== strategy || $[58] !== terms ? (t17 = () => {
		let orderingChanged = !isEqual(ordering, previousOrderingRef.current), cursorChanged = cursor !== previousCursorRef.current, termsChanged = !isEqual(terms, previousTermsRef.current);
...
//# sourceMappingURL=SearchProvider-BoC91Z5A.js.map