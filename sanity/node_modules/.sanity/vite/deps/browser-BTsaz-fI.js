...
//#region node_modules/event-source-polyfill/src/eventsource.js
var require_eventsource = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	(function(global) {
		...
		if (Object.create == void 0) Object.create = function(C) {
			function F() {}
			F.prototype = C;
			return new F();
		};
		...
		TextDecoderPolyfill.prototype.decode = function(octets) {
			function valid(codePoint, shift, octetsCount) {
				if (octetsCount === 1) return codePoint >= 128 >> shift && codePoint << shift <= 2047;
				if (octetsCount === 2) return codePoint >= 2048 >> shift && codePoint << shift <= 55295 || codePoint >= 57344 >> shift && codePoint << shift <= 65535;
				if (octetsCount === 3) return codePoint >= 65536 >> shift && codePoint << shift <= 1114111;
				throw new Error();
			}
			...
			for (var i = 0; i < octets.length; i += 1) {
				...
				if (bitsNeeded === 0) {
					if (codePoint <= 65535) string += String.fromCharCode(codePoint);
					else {
		...
		if (TextDecoder == void 0 || TextEncoder == void 0 || !supportsStreamOption()) TextDecoder = TextDecoderPolyfill;
		var k = function() {};
		...
		XHRWrapper.prototype.open = function(method, url) {
			...
			var onFinish = function(type, event) {
				if (event == null || event.preventDefault == null) event = { preventDefault: k };
				onProgress();
				if (state === 1 || state === 2 || state === 3) {
					state = 4;
					if (timeout !== 0) {
			...
			var onReadyStateChange = function(event) {
				if (xhr != void 0) {
					if (xhr.readyState === 4) {
						...
						if (!("onprogress" in xhr)) onProgress();
			...
			if ("readyState" in xhr) timeout = setTimeout(function() {
				onTimeout();
			}, 0);
		};
		...
		XHRTransport.prototype.open = function(xhr, onStartCallback, onProgressCallback, onFinishCallback, url, withCredentials, headers) {
			...
			xhr.onerror = function(event) {
				...
				onFinishCallback(/* @__PURE__ */ new Error("NetworkError"));
			};
			xhr.onload = function() {
			...
			xhr.onreadystatechange = function() {
				if (xhr.readyState === XMLHttpRequest.HEADERS_RECEIVED) {
					...
					onStartCallback(status, statusText, xhr.getResponseHeader("Content-Type"), new HeadersPolyfill(xhr.getAllResponseHeaders()));
				}
			};
				...
				return new Promise(function(resolve, reject) {
					var readNextChunk = function() {
						reader.read().then(function(result) {
							if (result.done) resolve(void 0);
							else {
								onProgressCallback(textDecoder.decode(result.value, { stream: true }));
								readNextChunk();
							}
		...
		function start(es, url, options) {
			...
			var onStart = function(status, statusText, contentType, headers) {
				if (currentState === CONNECTING) {
					if (status === 200 && contentType != void 0 && contentTypeRegExp.test(contentType)) {
						...
						} else message = "EventSource's response has a Content-Type specifying an unsupported type: " + (contentType == void 0 ? "-" : contentType.replace(/\s+/g, " ")) + ". Aborting the connection.";
						close();
			...
			var onFinish = function(error) {
				if (currentState === OPEN || currentState === CONNECTING) {
					...
					es.dispatchEvent(event);
					fire(es, es.onerror, event);
					if (error != void 0) console.error(error);
				}
			};
			var close = function() {
				currentState = CLOSED;
			...
			var onTimeout = function() {
				...
				if (currentState !== WAITING) {
					if (!wasActivity && abortController != void 0) {
						onFinish(/* @__PURE__ */ new Error("No activity within " + heartbeatTimeout + " milliseconds. " + (currentState === CONNECTING ? "No response received." : textLength + " chars received.") + " Reconnecting."));
						if (abortController != void 0) {
							abortController.abort();
							abortController = void 0;
						}
		...
		if (XMLHttpRequest != void 0 && (NativeEventSource == void 0 || !("withCredentials" in NativeEventSource.prototype))) R = EventSourcePolyfill;
		(function(factory) {
...
var browser_default = (/* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
