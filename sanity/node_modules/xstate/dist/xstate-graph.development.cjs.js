...
var StateMachine = require('./StateMachine-3778f5e0.development.cjs.js');
var dist_xstateGuards = require('./raise-53e20da7.development.cjs.js');
require('./xstate-dev.development.cjs.js');
require('./assign-f03a9bed.development.cjs.js');
  ...
  constructor(testLogic, options) {
    ...
    this._toTestPath = statePath => {
      ...
      return {
        ...
        description: dist_xstateGuards.isMachineSnapshot(statePath.state) ? `Reaches ${getDescription(statePath.state).trim()}: ${eventsString}` : JSON.stringify(statePath.state)
      };
    };
    this.options = {
...
function createTestModel(machine, options) {
  ...
  const testModel = new TestModel(machine, {
    ...
    events: state => {
      ...
      return dist_xstateGuards.getAllOwnEventDescriptors(state).flatMap(eventType => {
...
function createMockActorScope() {
  const emptyActor = dist_xstateActors.createEmptyActor();
  return {
    self: emptyActor,
    logger: console.log,
    id: '',
...
function createDefaultMachineOptions(machine, options) {
  ...
  const traversalOptions = {
    ...
    events: state => {
      const events = typeof getEvents === 'function' ? getEvents(state) : getEvents ?? [];
      return dist_xstateGuards.getAllOwnEventDescriptors(state).flatMap(type => {
