...
import { PaginatedGetBackupStream } from '../../actions/backup/fetchNextBackupPage.js';
...
${styleText('bold', 'backupId')}: ${styleText('cyan', opts.backupId)}`, {
        ...
        try {
            const backupFileStream = new PaginatedGetBackupStream(opts.projectId, opts.datasetName, opts.backupId);
            const files = [];
            let i = 0;
            ...
            this.error(`Downloading dataset backup failed: ${message}`, {
                exit: exitCodes.RUNTIME_ERROR
            });
        }
        docOutStream.end();
        ...
        try {
            ...
            this.error(`Archiving backup failed: ${message}`, {
                exit: exitCodes.RUNTIME_ERROR
            });
        }
        progressSpinner.set({
    ...
    async getOutputPath(defaultOutFileName) {
    ...
    async prepareBackupOptions(projectId, datasetName) {
        ...
        const backupId = String(this.flags['backup-id'] || await this.promptForBackupId(projectId, datasetName));
        if ('concurrency' in this.flags && (this.flags.concurrency < 1 || this.flags.concurrency > MAX_DOWNLOAD_CONCURRENCY)) {
    ...
    async promptForBackupId(projectId, datasetName) {
        ...
        try {
            ...
            const hint = backupIdChoices.length === maxBackupIdsShown ? ` (only last ${maxBackupIdsShown} shown)` : '';
            return select({
