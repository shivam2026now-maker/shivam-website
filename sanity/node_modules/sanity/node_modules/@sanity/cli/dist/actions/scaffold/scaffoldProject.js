...
const STUDIO_PACKAGE_MANAGERS = new Set([
    'npm',
    'pnpm',
    'yarn'
]);
export async function isStudioScaffoldTargetAvailable(workDir) {
    return isScaffoldTargetAvailable(workDir, STUDIO_DIR);
}
async function isScaffoldTargetAvailable(workDir, targetDir) {
...
async function inspectScaffoldTargets(workDir) {
    if (!await isStudioScaffoldTargetAvailable(workDir)) {
        return {
            unavailableTarget: STUDIO_DIR
        };
    }
    ...
    if (!detectedFramework && !await isScaffoldTargetAvailable(workDir, FRONTEND_DIR)) {
        return {
            unavailableTarget: FRONTEND_DIR
        };
    }
    return {
        detectedFramework
...
export async function getUnavailableScaffoldTarget(workDir) {
    const { unavailableTarget } = await inspectScaffoldTargets(workDir);
    return unavailableTarget;
}
function createStudioEnv(dataset, projectId, token) {
    return {
...
function createFrontendEnv(dataset, projectId) {
    return {
        NEXT_PUBLIC_SANITY_DATASET: dataset,
        NEXT_PUBLIC_SANITY_PROJECT_ID: projectId
    };
}
export async function scaffoldProject({ cancelSignal, dataset, displayName, output, packageManager, projectId, telemetry, token, workDir }) {
    ...
    const options = {
        ...
        packageManager: STUDIO_PACKAGE_MANAGERS.has(resolvedPackageManager) ? resolvedPackageManager : undefined,
        project: projectId,
        skillsMode: 'skip',
        template: 'clean',
        typescript: true,
    ...
    try {
        ...
        await initStudio({
            datasetName: dataset,
            defaults: await getProjectDefaults({
                isPlugin: false,
                workDir
            }),
            displayName,
    ...
    cancelSignal?.throwIfAborted();
    writeScaffoldEnv(path.join(studioPath, '.env.local'), createStudioEnv(dataset, projectId, token), `Studio scaffold completed, but writing ./${STUDIO_ENV_FILE} failed`, 'it holds a live project token');
    if (detected) {
    ...
    writeScaffoldEnv(path.join(frontendPath, '.env.local'), createFrontendEnv(dataset, projectId), `Website scaffold completed, but writing ./${FRONTEND_ENV_FILE} failed`, 'it holds the project ID and dataset');
    let frontendDependenciesInstalled = resolvedPackageManager !== 'manual';
    let frontendDependencyError;
...
function writeScaffoldEnv(envPath, values, errorContext, gitignoreReason) {
    const lines = [
    ...
    try {
        ...
        throw new Error(`${errorContext}: ${getErrorMessage(err)}`, {
            cause: err
        });
    }
}
