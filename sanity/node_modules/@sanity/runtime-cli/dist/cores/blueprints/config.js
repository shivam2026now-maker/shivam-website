...
import { CancelledError, runScopeAndStackWizard } from '../../utils/display/prompt.js';
...
export async function blueprintConfigCore(options) {
    ...
    if (!configStackId && !configScopeType && !configScopeId) {
        ...
        if (!editConfig) {
            log(`Run \`${runCommand(bin)} blueprints doctor\` for diagnostics.`);
    ...
    if (!editConfig) {
        ...
        let wizardResult;
        try {
            wizardResult = await runScopeAndStackWizard({
                token,
                knownOrganizationId,
                knownProjectId,
                logger: log,
            ...
            if (error instanceof CancelledError) {
                return { success: false, error: 'Configuration cancelled.', cancelled: true };
            }
            throw error;
        }
