...
const app = (host, port, validateResources, executionOptions) => {
    ...
    // Send a JSON envelope to every connected UI client.
    ...
    let nestedOrder = 0;
    /**
     * Re-entrant callback that resolves a nested `context.invoke(name, payload)` by
     * re-entering the invoke pipeline (which spawns its own child) and streams the
     * nested run to the UI. Passing `nestedInvoke` to itself keeps it recursable.
     */
    const nestedInvoke = async (name, payload, ctx, depth, callStack) => {
        const order = ++nestedOrder;
        const result = await handleInvokeRequest(name, payload, { event: 'nested' }, ctx, validateResources, executionOptions, depth, callStack, nestedInvoke);
        broadcast({
            type: 'nested-invoke',
            name,
            order,
    ...
    const requestListener = async (req, res) => {
        ...
        switch (true) {
                ...
                if (req.method === 'POST') {
                    ...
                    req.on('end', async () => {
                        ...
                        try {
                            ...
                            const { data, func: functionName, metadata, } = parseInvokeRequest(Buffer.concat(body), dirname(fileInfo.blueprintFilePath));
                            ...
                            const result = await handleInvokeRequest(functionName, event, metadata, context, validateResources, executionOptions, 0, [], nestedInvoke);
            ...
            case req.url === '/projects': {
                res.setHeader('Content-Type', 'application/json');
                try {
                    const response = await fetch(`${config.populusApiUrl}/v2021-06-07/projects?includeOrganizationProjects=true`, {
                        headers: {
            ...
            case req.url?.startsWith('/datasets'): {
                ...
                try {
                    let json = [];
                    if (projectId) {
                        const response = await fetch(`${config.populusApiUrl}/v2024-08-25/projects/${projectId}/datasets?includeStats=true`, {
                            headers: {
                                Authorization: `Bearer ${config.token}`,
                            },
                        });
            ...
            case req.url === '/organizations': {
                res.setHeader('Content-Type', 'application/json');
                try {
                    const response = await fetch(`${config.populusApiUrl}/v2021-06-07/organizations?includeImplicitMemberships=true&includeMembers=false&includeFeatures=false`, {
                        headers: {
                            Authorization: `Bearer ${config.token}`,
                        },
                    });
            ...
            case req.url?.startsWith('/media-libraries'): {
                ...
                res.setHeader('Content-Type', 'application/json');
                try {
                    const response = await fetch(`${config.populusApiUrl}/vX/media-libraries?organizationId=${organizationId}`, {
                        headers: {
                            Authorization: `Bearer ${config.token}`,
                        },
                    });
    ...
    wss.on('connection', function connection(ws) {
        ...
        if (!fileInfo)
            return;
        watchFile(fileInfo.blueprintFilePath, { interval: 2007 }, (curr, prev) => {
            // watchFile can also fire on access-time (atime); gate on modified-time (mtime)
...
function parseInvokeRequest(body, blueprintFileDir) {
    let json;
    try {
    ...
    if (typeof metadataEvent !== 'string' ||
        (!isEventType(metadataEvent) && metadataEvent !== 'plain')) {
    ...
    return {
        ...
        metadata: metadataEvent === 'plain' ? { event: metadataEvent } : { event: metadataEvent, before, after },
    };
}
