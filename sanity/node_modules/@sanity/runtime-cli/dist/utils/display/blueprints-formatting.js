...
import { SANITY_ACCESS_ROBOT, SANITY_ACCESS_ROLE, SANITY_FUNCTION_DOCUMENT, SANITY_FUNCTION_DURABLE, SANITY_FUNCTION_MEDIA_LIBRARY_ASSET, SANITY_FUNCTION_PUBSUB, SANITY_FUNCTION_SCHEDULED, SANITY_FUNCTION_SYNC_TAG_INVALIDATE, SANITY_PROJECT, SANITY_PROJECT_CORS, SANITY_PROJECT_DATASET, SANITY_PROJECT_WEBHOOK, SANITY_STUDIO, } from '../../constants.js';
import { styleText } from '../style-text.js';
import { isCorsOriginResource, isDatasetResource, isRobotResource, isRoleResource, isStudioResource, isWebhookResource, } from '../types.js';
import { formatDateTime, formatDuration } from './dates.js';
import { capitalize, niceId, renderSection } from './presenters.js';
import { arrayifyCors, arrayifyDataset, arrayifyFunction, arrayifyRobot, arrayifyRole, arrayifyStudio, arrayifyWebhook, } from './resources-formatting.js';
...
const RESOURCE_CATEGORIES = {
    ...
    [SANITY_STUDIO]: {
        ...
        displayNameAttribute: 'title',
        formatDetails(res) {
            return isStudioResource(res) ? arrayifyStudio(res) : [];
        },
    },
};
...
function deployedResourceName(res, displayNameAttribute, verbose = false) {
    const nameParts = [styleText(['bold', 'green'], res.name)];
    // deployed state: prefer the resolved value, fall back to the authored one
    const nameValue = displayNameAttribute
        ? (res.resolvedParameters?.[displayNameAttribute] ?? res.parameters[displayNameAttribute])
        : undefined;
    const displayName = typeof nameValue === 'string' && nameValue;
    if (displayName && displayName !== res.name)
        nameParts.push(styleText('green', `"${displayName}"`));
    if (verbose)
...
export function formatDeployedResourceTree(resources, verbose = false) {
    ...
    const output = buildOutputTree(resources, (category, res, v) => deployedResourceName(res, category.displayNameAttribute, v), 
    // deployed state: prefer resolved values, fall back to authored ones
    (res) => ({ name: res.name, type: res.type, ...(res.resolvedParameters ?? res.parameters) }), verbose);
...
const IGNORED_PARAMS = {
    ...
    [SANITY_FUNCTION_PUBSUB]: new Set(['src']),
